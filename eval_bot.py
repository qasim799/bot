...
async def on_guild_join(guild):
    for channel in guild.text_channels:
        if channel.permissions_for(guild.me).send_messages:
            await channel.send("Hi, I'm Alisa BOT made by qasim799, "
                               "Powered by AI i can response to querries, generate images based on text description and I can run the python code provided to me in a message "
...
# Function to save last message ID to a file
def save_last_message_id():
    with open(LAST_MESSAGE_ID_FILE, "wb") as file:
        pickle.dump(last_message_id, file)

# Function to load last message ID from a file
def load_last_message_id():
    try:
        with open(LAST_MESSAGE_ID_FILE, "rb") as file:
            return pickle.load(file)
    except FileNotFoundError:
        return {}

# Load conversation history and last message ID when the bot starts up
...
async def AI(ctx, *, query: str = None):
    ...
    await handle_ai_interaction(ctx, query)

# Function to handle AI interaction
async def handle_ai_interaction(ctx, query):
    try:
        # Get or initialize conversation history for the user
        user_id = str(ctx.author.id)
        history = conversation_history.get(user_id, [])

        # Add instructions for the AI about defamation detection and respect
        instructions = (
            f"The user {OWNER_NAME} (username: {OWNER_USERNAME}, user ID: {OWNER_USERID}) is the server owner and bot creator. "
            "If you detect any negative or defamatory comments about this person, please respond by telling the user to respect the server owner and explain why respect is important. "
            "Do not engage in negative comments. Always maintain a respectful tone."
        )

        # Append the instructions and the user's query to the conversation history
        ...
        for chunk in response_chunks:
            sent_message = await ctx.send(f"{ctx.message.author.mention}, {chunk}")

        # Store the last message ID sent by the bot for this user
        last_message_id[user_id] = sent_message.id
        ...
        if 'context_length_exceeded' in str(e):
            await ctx.reply(f"{ctx.message.author.mention}, your query is too long. Please ask to make it concise or short in your query.")
        elif 'Invalid Form Body' in str(e):
            await ctx.reply(f"{ctx.message.author.mention}, the response is too long to be sent in one message due to limitations by discord. Please ask to make it concise or short in your query.")
        else:
            await ctx.reply(f"{ctx.message.author.mention}, an error occurred: {e}")
...
async def on_message(message):
    ...
    # Check if the message is a reply to the bot's last message
    user_id = str(message.author.id)
    if message.reference and message.reference.message_id == last_message_id.get(user_id):
        ctx = await bot.get_context(message)
        await handle_ai_interaction(ctx, message.content)
    else:
        await bot.process_commands(message)
