...
# Command to generate image using advanced parameters
...
async def generate_image(ctx, *, prompt: str):
    try:
        async with aiohttp.ClientSession() as session:
            ...
            # Example payload with more parameters
            payload = {
                "textPrompt": prompt,
            }

            # Submit the prompt to the imagine parameter endpoint
            async with session.post(IMAGINE_PARAMETER_ENDPOINT, json=payload, headers=headers) as response:
                imagine_data = await response.json()
                print("Imagine Parameter Response Status:", response.status)
                print("Imagine Parameter Response Content:", imagine_data)

                if not imagine_data.get("taskId"):
                    await ctx.reply(f"Failed to initiate image generation. Error: {imagine_data.get('error', 'Unknown error')}")
                    return
                ...
                while True:
                    await asyncio.sleep(15)  # Wait for 5 seconds before polling again
